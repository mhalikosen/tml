...
function parseInterpolations(text: string): TextSegment[] {
  ...
  while (remaining.length > 0) {
    ...
    const isRaw = rawIndex !== -1 && (escapedIndex === -1 || rawIndex <= escapedIndex);
    ...
    if (isRaw) {
      ...
      if (escapedIndex > 0) {
        segments.push({ type: "text", value: remaining.slice(0, escapedIndex) });
      }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (match) {
      if (blockStack[blockStack.length - 1] !== "if") {
        throw new TmlCompileError("@elseif without matching @if", filePath, lineNum);
      }
    ...
    if (DIRECTIVE_ELSE.test(trimmed)) {
      if (blockStack[blockStack.length - 1] !== "if") {
        throw new TmlCompileError("@else without matching @if", filePath, lineNum);
      }
    ...
    if (DIRECTIVE_END.test(trimmed)) {
      ...
      if (!block) {
        throw new TmlCompileError("Unexpected @end without matching block", filePath, lineNum);
      }
      if (block === "each") {
